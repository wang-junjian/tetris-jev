...
- [x] **5. 判断链**：strategy → placement 两跳串行——先问 strategy，
  把 Jev 自己的回答喂回 placement 问题的 state 再问落点。代码只传话。
  **已实现并验证为回退**（2026-09-23，seeds 0-1，max-pieces 200，
  日志 `eval/runs/jev-v4.jsonl`，诊断 `eval/runs/jev-v4-diagnosis.md`）：
  **43 / 58 行（均值 50.5，-29%）且两局均 top-out**（v3 同口径 73 / 70 行打满 200 块）；
  劣招率 2.8%→5.1%，I 块劣招率 5.4%→10.6%（P1 复发），放弃消行 2 次→5 次（P2 复发）。
  机制：喂回的策略文本（repair_surface「即使不消行」、survive「即使难看」）与
  priorities/显著事实竞争权重，把第 4 步修好的靶子又遮住了；P4 的策略分布偏斜
  （clear_lines 仅 4/332）使喂回文本大多不利于消行。
  **结论**：判断链降级为实验开关（`--chain 1` / 页面复选框），默认保持 v3 单请求。
  若后续重启此方向，需先重做策略选项词化（P4：去掉反消行措辞、合并模糊边界）。
